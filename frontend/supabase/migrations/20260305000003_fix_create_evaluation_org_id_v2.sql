-- Fix: organization_id still NULL because supplier has no org set.
-- Strategy: supplier org_id → user profile org_id → NULL (make column nullable).

-- 1. Make organization_id nullable so it never hard-fails
ALTER TABLE public.evaluations
  ALTER COLUMN organization_id DROP NOT NULL;

-- 2. Update function: supplier org first, profile org as fallback
...
DECLARE
  ...
  v_org_id        uuid;
BEGIN
  -- Try supplier org first, fall back to caller's profile org
