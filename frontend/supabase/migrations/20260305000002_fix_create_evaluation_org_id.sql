-- Fix: evaluations.organization_id NOT NULL violation.
-- The live DB has organization_id NOT NULL on evaluations (added outside migrations).
-- Derive it from the supplier record so no frontend changes are needed.
...
DECLARE
  ...
  v_org_id        uuid;
BEGIN
  -- Derive organization_id from the supplier
